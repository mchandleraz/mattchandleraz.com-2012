	...
	*/
    ...
    if (responsive_viewport < 768) {
    	...
    	$('#menuToggle').click(function(e){
		    $('#nav, #menuToggle, #content, footer').toggleClass('active');
		    // if menuToggle is active:
		    if ($('#menuToggle').hasClass('active')) {
			    var height = $(document).height();
				$('#nav.active').height(height);
				$('#content.active').css('margin-top', '-'+height+'px');
				$('footer.active').css({'position': 'absolute', 'bottom': '0'});
			};
			// if menuToggle is not active:
		    if (!$('#menuToggle').hasClass('active')) {
				$('#content').css('margin-top', ''); // strip margin-top inline style
			};
		});
	...
	if (responsive_viewport > 768) {
		...
		*/
		...
		$('.portfolio-item img').click(function() {
			if ( !$(this).parents('.portfolio-item').hasClass('open') ) {
		    	...
		    	$(this).parents('.portfolio-item').toggleClass('open');
	    	};
	  	});
